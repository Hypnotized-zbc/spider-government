...
### 修复：站点改版导致标题/项目编号提取失败（"无标题"问题）
- 根因：cqggzy.com 详情页 2026-08 改版（Vue 重构），页面不再有 `<h1>`——
  旧版 extract_detail 只从 h1 提取标题与项目编号，改版后全部落空：
  - 标题固定为"无标题"，项目编号为空 → md/docx/报表全部缺标题缺编号
  - 多条公告同名"无标题" → safe_name 文件名互相覆盖，zip 内重复条目、数量与输入不一致
- 修复：
  - extract_detail 适配新结构：标题取 `.detail-wrapper` 内 `div[class*=text-2xl]` 首块，
    项目编号取紧随其后的 `div.text-2xl`（兼容全角/半角冒号）；保留旧版 h1 分支兼容；
    兜底取正文区 `.gonggaotitle` 并去除"招标公告"等后缀
  - make_summary_zip 增加去重：同名记录（标题相同 → 文件名相同）不再重复写入 zip
- 数据清理：剔除 8/17、8/18 三条"无标题"脏记录及对应 md/docx/附件，重爬验证
- 验证：重爬 8/17–8/18 共 16 条，标题/编号/信息时间全部提取正确（真实站点）
